...
<template>
  <div>
    <ul class="nav">
      ...
      <li style="position: absolute;right: 480px;">
        <span @click="healthDataRecord" style="margin:14px 10px;">
          <i class="el-icon-edit-outline"></i>
          健康指标记录
        </span>
      </li>
      <li style="position: absolute;right: 630px;">
        <span @click="dietHistoryRecord" style="margin:14px 10px;">
          <i class="el-icon-edit-outline"></i>
          饮食记录
        </span>
      </li>
...
export default {
  ...
  methods: {
    // 搜索关键字，返回父组件处理
  search() {
    // 如果当前是搜索页面，更新关键字并触发搜索
    if (this.$route.path === "/search-detail") {
      // 触发事件，通知搜索页的 keyword 发生变化
      this.$bus.$emit("updateKeyword", this.filterText);
      return;
    }
    // 将关键词存起来
    ...
    dietHistoryRecord() {
      this.$emit("eventListener", "dietHistoryRecord");
    },
    async loadMsgCount() {
