...
<template>
  <el-row style="background-color: #FFFFFF;padding: 10px 0;border-radius: 5px;">
    <el-row style="padding: 10px 0 20px;border-bottom: 1px solid #f1f1f1;margin: 0 10px;">
      <el-row>
        <el-select size="small" style="width: 188px;margin-right: 10px;" v-model="queryParams.detail"
          placeholder="选择记录项">
    ...
    <el-row style="margin: 10px;">
      <el-table row-key="id" @selection-change="handleSelectionChange" :data="tableData" style="width: 100%"  class="custom-table">
        ...
        <el-table-column prop="avatar" width="200" label="头像">
          <template slot-scope="scope">
            <el-avatar shape="square" :size="40" :src="scope.row.userAvatar" style="margin-top: 10px;"></el-avatar>
          </template>
        </el-table-column>
        <el-table-column prop="userName" width="200" label="记录者"></el-table-column>
        <el-table-column prop="recipeName" width="300" label="食谱名称"></el-table-column>
        <el-table-column prop="dietName" width="120" label="餐"></el-table-column>
   
        <el-table-column prop="energy" width="200" label="能量值" header-align="center"></el-table-column>
  ...
  methods: {
    ...
    async batchDelete() {
      ...
      try {
        ...
        const response = await this.$axios.post("/diet-history/delete", ids);
        if (response.data.code === 200) {
