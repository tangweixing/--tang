...
<template>
  <el-row style="background-color: #FFFFFF;padding: 10px 0;border-radius: 5px;">
    <el-row style="padding: 10px 0 20px;border-bottom: 1px solid #f1f1f1;margin: 0 10px;">
      <el-row>
        <el-select size="small" style="width: 188px;margin-right: 10px;" v-model="recipeQueryDto.typeId"
          placeholder="选择记录项">
          <el-option v-for="option in typeOptions" :key="option.value" :label="option.label"
            :value="option.value"></el-option>
        </el-select>
        <span class="dialog-hover">是否审核</span>
        <el-radio v-model="recipeQueryDto.isAudit" :label="false">否</el-radio>
        <el-radio v-model="recipeQueryDto.isAudit" :label="true">是</el-radio>
        <span class="top-bar">食谱名称</span>
        <el-input size="small" style="width: 188px;" v-model="recipeQueryDto.name" placeholder="输入处" clearable
    ...
    <el-row style="margin: 10px;">
      <el-table row-key="id" @selection-change="handleSelectionChange" :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="typeName" width="200" label="类型"></el-table-column>
        <el-table-column width="200" label="是否审核">
          <template slot-scope="scope">
            <!-- scope.row 是当前行数据 -->
            <span :class="scope.row.isAudit ? 'text-success' : 'text-danger'">
    ...
    <!-- 操作面板 -->
    <el-drawer :show-close="false" :visible.sync="dialogrecipeOperaion" width="34%">
      <div slot="title">
      ...
      <div style="padding:0 20px;">
        <el-row>
          ...
          <div>
            <el-select size="small" style="width: 188px;margin-right: 10px;" v-model="data.typeId" placeholder="选择记录项">
              <el-option v-for="option in typeOptions" :key="option.value" :label="option.label"
                :value="option.value"></el-option>
            </el-select>
          </div>
      ...
      <div class="dialog-footer">
        ...
        <el-button class="customer" size="small" style="background-color: rgb(241, 241, 241);border: none;"
          @click="dialogrecipeOperaion = false">取消</el-button>
      </div>
      <!-- </span> -->
    </el-drawer>
  </el-row>
...
export default {
  ...
  data() {
    return {
      data: { cover: "", isAudit: true }, // 初始化封面为空，避免undefined
      currentPage: 1,
      pageSize: 7,
      totalItems: 0,
      dialogrecipeOperaion: false, // 操作弹窗开关
      isOperation: false, // 标识：false=新增，true=编辑
      tableData: [],
      searchTime: [], // 时间范围搜索
      selectedRows: [], // 表格选中行（批量删除用）
      recipeQueryDto: { isAudit: null }, // 搜索条件DTO
      userIdList: [],
      switchValue: true,
      typeOptions: []
    };
  ...
  methods: {
    ...
    async updateOperation() {
      try {
        ...
        if (response.data.code === 200) {
          this.dialogrecipeOperaion = false; // 关闭弹窗
          this.fetchFreshData(); // 重新加载列表
        }
      } catch (error) {
    ...
    async fetchFreshData() {
      try {
        ...
        console.log(this.recipeQueryDto.isAudit);
    ...
    // 标题输入框清空回调
    handleFilterClear() {
      this.recipeQueryDto.title = ""; // 明确清空标题，避免filterText冗余
      this.handleFilter();
    },
    // 分页-每页条数变更
