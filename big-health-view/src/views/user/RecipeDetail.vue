  ...
  <div class="news-detail">
    ...
    <div class="content-container">
      ...
      <div class="main-content">
        ...
        <p class="publish-time">发布时间：{{ recipeDetail.createTime }}</p>
        <div class="flow-data">
        ...
        <!-- 封面图 -->
        <img :src="recipeDetail.cover" alt="资讯封面" class="news-cover" v-if="recipeDetail.cover" />

        <!-- 资讯内容 -->
        <div class="news-content" v-html="recipeDetail.content"></div>
        <FlowIndex contentModule="RECIPE" :contentId="this.id" />
        <!-- 评论点赞组件 -->
        <Evaluations :contentId="recipeDetail.id" contentType="news" />
      </div>
      ...
      <div class="recommend-sidebar">
        ...
        <div class="recommend-list">
          <!-- 推荐资讯项 -->
          <div class="recommend-item" v-for="item in recommendList" :key="item.id" @click="goTorecipeDetail(item.id)">
            <img :src="item.cover" alt="推荐资讯封面" class="recommend-cover" v-if="item.cover" />
            <div class="recommend-info">
              <p class="recommend-title">{{ item.name }}</p>
              <p class="recommend-time">{{ item.createTime }}</p>
...
export default {
  ...
  methods: {
    ...
    async fetchrecipeDetail(id) {
      try {
        const response = await this.$axios.get(`/recipe/${id}`);
